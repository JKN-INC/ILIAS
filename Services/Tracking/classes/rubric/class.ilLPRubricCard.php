...
class ilLPRubricCard
{
    ...
    public function copy($new_obj_id) 
    {
        $groups = [];
        $labels = [];

        $l_res = $this->ilDB->query(
            "SELECT * FROM rubric_label 
                WHERE rubric_id=".$this->ilDB->quote($this->rubric_id, "integer")." AND deleted IS NULL"
        );

        while ($l_row = $l_res->fetchRow(ilDBConstants::FETCHMODE_ASSOC)) {
            array_push($labels, ['label' => $l_row['label']]);
        }

        $g = 1;
        $g_res = $this->ilDB->query(
            "SELECT * FROM rubric_group 
                WHERE rubric_id=".$this->ilDB->quote($this->rubric_id, "integer")." AND deleted IS NULL"
        );

        while($g_row = $g_res->fetchRow(ilDBConstants::FETCHMODE_ASSOC)) {
            $groups[$g] = ['group_name' => $g_row['group_name']];
            $groups[$g]['weights'] = [];
            $w_res = $this->ilDB->query(
                "SELECT * FROM rubric_weight 
                    WHERE rubric_group_id=".$this->ilDB->quote($g_row['rubric_group_id'], "integer").
                    " AND deleted IS NULL"
            );

            while ($w_row = $w_res->fetchRow(ilDBConstants::FETCHMODE_ASSOC)) {
                array_push($groups[$g]['weights'], $w_row['weight_min'] . "-" . $w_row['weight_max']);
            }

            $c = 1;
            $c_res = $this->ilDB->query(
                "SELECT * FROM rubric_criteria
                    WHERE rubric_group_id=".$this->ilDB->quote($g_row['rubric_group_id'], "integer").
                    " AND deleted IS NULL"
            );

            while ($c_row = $c_res->fetchRow(ilDBConstants::FETCHMODE_ASSOC)) {
                if (!is_array($groups[$g]['criteria'])) {
                    $groups[$g]['criteria'] = [];
                }
                $groups[$g]['criteria'][$c] = ['criteria_name' => $c_row['criteria']];
                
                $b_res = $this->ilDB->query(
                    "SELECT * FROM rubric_behavior
                        WHERE rubric_criteria_id=".$this->ilDB->quote($c_row['rubric_criteria_id'], "integer").
                        " AND deleted IS NULL"
                );

                $b = 1;
                while ($b_row = $b_res->fetchRow(ilDBConstants::FETCHMODE_ASSOC)) {
                    $groups[$g]['criteria'][$c]['behavior'][$b] = ['behavior_name' => $b_row['description']];
                    $b++;
                }
                $c++;
            }
        ...
        $labels = $this->saveRubricLabelTbl($labels);
        $this->saveRubricGroupTbl($groups, $labels);
        $this->saveRubricCriteriaTbl($groups);
        $this->saveRubricBehaviorTbl($groups, $labels);
    }
     ...
     * @return int
     *
     * A rubric card is in Grader Mode when at least one student has been graded.
     * Once the rubric is in Grader Mode, it cannot be placed back into Developer Mode.
     * In Grader Mode, groups, criteria and behavior cannot be added or taken away.
     * Only text based changes can be made to the group and criteria label,and the behavior description.
     */
