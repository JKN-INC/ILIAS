...
class ilLPRubricGrade
{
    ...
    public function lockUnlockGrade()
    {
        if (!$this->isGradingLocked()) {
            $this->ilDB->manipulateF(
                'INSERT INTO rubric_grade_lock(grade_lock_id,rubric_id,user_id,owner,create_date,last_update)
            VALUES (%s,%s,%s,%s,%s,%s)',
                array("integer", "integer", "integer", "integer", "timestamp", "timestamp"),
                array(
                    $this->ilDB->nextId('rubric_grade_lock'), $this->rubric_id, $this->getGradeUserId(),
                    $this->user->getId(), date("Y-m-d H:i:s"), date("Y-m-d H:i:s")
                )
            );
        } else {
            $this->ilDB->manipulate(
                "DELETE FROM rubric_grade_lock WHERE rubric_id = " . $this->ilDB->quote($this->rubric_id, "integer")
     ...
     * @param $obj_id
     */
    public function sendRubricNotification($user_id, $obj_id)
    {
        include_once("./Services/Tracking/classes/rubric/class.ilRubricGradeNotification.php");
        $not = new ilRubricGradeNotification();
        $not->setObjId($obj_id);
        $not->setRecipients(array($user_id));
        $not->send();
    }
    ...
    public function getRubricComment($user_id, $history_id = NULL)
    {
        if (!is_null($history_id) && $history_id !== 'current') {
            $history = $this->_lookupRubricHistoryLP($history_id);
            $comments = $history['comments'];
        } else {
            include_once 'Services/Tracking/classes/class.ilLPMarks.php';
            $marks = new ilLPMarks($this->obj_id, $user_id);
    ...
    public function getUserHistory($user_id)
    {
        $history = array();
        $res = $this->ilDB->query(
            "SELECT * FROM (select h.rubric_history_id,d.create_date,d.owner from rubric_grade_hist h INNER JOIN rubric_data d on d.deleted = h.create_date WHERE h.deleted IS NULL AND h.obj_id = " . $this->ilDB->quote($this->obj_id, "integer") .
    ...
    public static function _prepareForRegrade($obj_id, $usr_id)
    {
        ...
        $affected_rows = $DIC["ilDB"]->manipulate("UPDATE rubric_data d INNER JOIN rubric r on d.rubric_id = r.rubric_id SET d.deleted =
                                            " . $DIC["ilDB"]->quote($delete_date, "timestamp") . " WHERE d.deleted IS NULL AND d.usr_id = "
            . $DIC["ilDB"]->quote($usr_id, "integer") . " AND r.obj_id = " .
            $DIC["ilDB"]->quote($obj_id, "integer"));
        if ($affected_rows > 0) {
            $ass_id = null;
            ...
            $id = $DIC["ilDB"]->nextID('rubric_grade_hist');
            $DIC["ilDB"]->manipulateF(
                "INSERT INTO rubric_grade_hist(rubric_history_id,rubric_id,obj_id,usr_id,status,mark,completed,comments,owner,create_date,last_update) VALUES " .
                    " (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
                array("integer", "integer", "integer", "integer", "integer", "float", "integer", "text", "integer", "date", "date"),
                array($id, self::_lookupRubricId($obj_id), $obj_id, $usr_id, $status, $mark, $completed, $comments, $DIC->user()->getId(), $delete_date, $delete_date)
            );
            ...
            if ($ass_id) {
                $assignmentMember = new ilExAssignmentMemberStatus($ass_id['id'], $usr_id);
                $assignmentMember->setMark('');
                $assignmentMember->setStatus('notgraded');
    ...
    public static function _lookupRubricId($obj_id)
    {
        global $DIC;
        $res = $DIC["ilDB"]->query(
            "select rubric_id from rubric where obj_id=" . $DIC["ilDB"]->quote($obj_id, "integer") . " and deleted is null"
        );
        $row = $res->fetchRow(ilDBConstants::FETCHMODE_OBJECT);
    ...
    public static function _lookupRubricHistoryLP($rubric_history_id)
    {
        global $DIC;
        $res = $DIC["ilDB"]->query(
            "select * from rubric_grade_hist where rubric_history_id=" . $DIC["ilDB"]->quote($rubric_history_id, "integer") . " and deleted is null"
        );
