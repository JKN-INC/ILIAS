var RUBRIC = {
  ...
  updateGrade: function () {
    ...
    tfoot[0].children[0].children[1].innerHTML =
      overall_total + " out of " + overall_max;
  ...
  getSingleRange: function (range) {
    ...
    var min = 10000000;
    var max = 0;
    if (broken_range[0] < min) min = broken_range[0];

    if (broken_range[0] > max) max = broken_range[0];

    if (broken_range[1] < min) min = broken_range[1];

    if (broken_range[1] > max) max = broken_range[1];
      ...
      for (var a = 0; a < inputs.length; a++) {
        ...
        for (var b = 0; b < inputs.length; b++) {
          if (inputs[b] !== inputs[a]) {
            ...
            if (
              current_range["low"] >= tmp_range["low"] &&
              current_range["low"] <= tmp_range["high"]
            ) {
              ...
              span2.setAttribute(
                "style",
                "text-align:center;color:red;display:block;font-size:75%;"
              );
            ...
            if (
              current_range["high"] >= tmp_range["low"] &&
              current_range["high"] <= tmp_range["high"]
            ) {
              var div = inputs[b].parentNode;
              div.classList.remove("has-warning");
              div.classList.remove("has-success");
    ...
    for (var a = 0; a < trs.length; a++) {
      if (this.nodeHasSlider(trs[a])) {
        ...
        for (var b = 0; b < total_behaviors; b++) {
          tmp_range = document.getElementById(
            "Points" + groups + "_" + b
          ).value;
          document.getElementById("Points" + groups + "_" + b).value =
            tmp_range; //update the input field
  ...
  delBehavior: function (thead, tbody, tfoot, position) {
    ...
    //remove tfoot
    tfoot.children[0].lastElementChild.colSpan =
      tfoot.children[0].lastElementChild.colSpan - 1;
  },
  ...
  addBehavior: function (thead, tbody, tfoot, position) {
    ...
    // thead changes
    th.appendChild(
      this.createCardFormHeadInputsDiv(txt_label, label_value, position)
    );
  ...
  nodeHasGrade: function (tr) {
    ...
    if (
      inputs.length == 1 &&
      inputs[0].getAttribute("placeholder") == "Grade"
    ) {
      return true;
    } else {
      return false;
  ...
  nodeHasGroupRange: function (tr) {
    if (
      tr.children[0].nodeName == "TH" &&
      tr.children[1].nodeName == "TH" &&
      tr.children[1].innerHTML == "Range"
    ) {
      return true;
    } else {
      return false;
  ...
  addPointSlider: function (group_number, behavior_number) {
    ...
    div.setAttribute(
      "class",
      "form-group has-warning has-feedback point-input"
    );
    ...
    input.setAttribute(
      "class",
      "Points" + group_number + "_" + behavior_number
    );
    ...
    input.setAttribute(
      "value",
      "" +
        point_ranges[behavior_number][0] +
        "-" +
        point_ranges[behavior_number][1] +
        ""
    );
    ...
    span.setAttribute(
      "id",
      "Points" + group_number + "_" + behavior_number + "WarningStatus"
    );
  ...
  addGroup: function () {
    ...
    tr.appendChild(
      this.createCardFormBodyInputs("Group Name", trs.length - 1, 0, true)
    );
    tr.appendChild(
      this.createCardFormBodyInputs("Criteria Label", trs.length - 1, 0, true)
    );
  ...
  delCriteria: function () {
    ...
    if (this.nodeHasGroupName(trs[selected_criteria])) {
      ...
      if (this.nodeHasPointRange(trs[selected_criteria].nextElementSibling)) {
        //delete point range
        trs[selected_criteria].nextElementSibling.parentNode.removeChild(
          trs[selected_criteria].nextElementSibling
        );
        ...
        td.setAttribute(
          "rowspan",
          trs[selected_criteria].children[0].rowSpan - 1
        );
        td.innerHTML = trs[selected_criteria].children[0].innerHTML;
        //insert the new td
        trs[selected_criteria + 1].insertBefore(
          td,
          trs[selected_criteria].nextSibling.firstChild
        );
        //delete the old row
        trs[selected_criteria].parentNode.removeChild(trs[selected_criteria]);
      }
  ...
  addCriteria: function () {
    ...
    tr.setAttribute("class", "tblrow1 small");
    tr.appendChild(
      this.createCardFormBodyInputs("Criteria Label", trs.length, 0, true)
    );
  ...
  getOneSelected: function (trs, looking_for) {
    ...
    for (var a = 0; a < trs.length; a++) {
      ...
      for (var b = 0; b < elems.length; b++) {
        if (
          elems[b].type == "checkbox" &&
          elems[b].id.toLowerCase().substr(0, 5) == looking_for &&
          elems[b].checked == true
        ) {
  ...
  createCardFormHeadInputsDiv: function (txt_label, txt_placeholder, position) {
    ...
    input.setAttribute(
      "aria-describedby",
      txt_label + position + "WarningStatus"
    );
    input.setAttribute("onkeyup", "validate(this)");
    input.setAttribute("oninput", "validate(this)");
    input.setAttribute("onblur", "recalculate(this)");
  ...
  createCardFormBodyInputs: function (
    txt_label,
    int_row_count,
    int_iteration,
    input_has_checkbox
  ...
  verifyForm: function () {
    ...
    var errors = document.querySelectorAll(
      ".glyphicon-remove,.glyphicon-warning-sign"
    );
    ...
    for (var a = 0; a < inputs.length; a++) {
      ...
      if (inputs[a].type == "text") {
        ...
        if (requires_verification === true) {
          if (
            verified_object === false ||
            verified_object.childNodes[0].nodeValue == "(error)"
          ) {
    ...
    verified_object =
      document.getElementById("passing_grade").parentNode.children[3];
  ...
  verifyGrade: function () {
    ...
    for (var a = 0; a < inputs.length; a++) {
      if (inputs[a].type == "text" && inputs[a].id.substr(0, 5) == "Grade") {
        ...
        if (
          isNaN(inputs[a].value) ||
          test_value > max ||
          test_value < min ||
          !RUBRIC.isNumeric(test_value)
        ) {
  ...
  fixids: function (td, type, g, c, b) {
    ...
    if (type == "group" || type == "criteria") {
      ...
      td.children[0].children[1].children[0].children[0].setAttribute(
        "id",
        tmpname + "_checkbox"
      ); // input checkbox
      ...
      td.children[0].children[1].children[1].setAttribute(
        "aria-describedby",
        tmpname + "_WarningStatus"
      ); // input text
      ...
      td.children[0].children[1].setAttribute(
        "aria-describedby",
        tmpname + "_WarningStatus"
      ); // input text
      td.children[0].children[3].setAttribute("id", tmpname + "_WarningStatus"); // span
...
function verifyForm() {
  ...
  try {
    ...
    document
      .getElementById("rubric_error_message")
      .setAttribute(
        "style",
        "background-color:rgb(241,221,221);border-radius: 1px;padding:5px;"
      );
    return false;
  }
}
...
function validate(obj) {
  ...
  if (validated === true) {
    ...
    modified_object.children[2].setAttribute(
      "class",
      "glyphicon glyphicon-ok form-control-feedback"
    );
...
document.addEventListener("DOMContentLoaded", function (event) {
  ...
  var rubric_inputs = rubric.querySelectorAll(
    "input.form-control,textarea.form-control"
  );
  for (var i = 0; i < rubric_inputs.length; i++) {
    if (
      rubric_inputs[i].getAttribute("placeholder") !== "Grade" &&
